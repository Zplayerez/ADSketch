...
# 创建输出目录
output_dir = "data/cpu_usage_benchmark"
os.makedirs(output_dir, exist_ok=True)

def generate_cpu_usage_data(num_points, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    # 生成正常范围的CPU使用率（2-5%）
    values = np.random.uniform(2.0, 5.0, num_points)
    
    # 添加较小的随机波动（标准差0.5%，保持相对波动幅度）
    values += np.random.normal(0, 0.5, num_points)
    
    # 确保所有值都在0-100之间（CPU使用率的有效范围）
    ...
    for i in range(num_anomaly_periods):
        ...
        # 90%概率是高CPU使用率异常，10%概率是CPU挂起异常（接近0%）
        if np.random.random() < 0.9:  # 高CPU使用率异常的概率更大
            # 高CPU使用率异常 (10-20%)，在基准值附近波动
            base_value = high_anomaly_bases[i]
            values[anomaly_start:anomaly_start + anomaly_duration] = base_value + np.random.uniform(-1, 1, anomaly_duration)
        else:
            # CPU挂起异常 (0-0.5%)
            values[anomaly_start:anomaly_start + anomaly_duration] = np.random.uniform(0, 0.5, anomaly_duration)
...
for i in range(1, num_files + 1):
    ...
    df = generate_cpu_usage_data(num_points, seed=i)  # 使用文件索引作为随机种子
    
    # 保存为CSV文件
    output_file = os.path.join(output_dir, f'cpu_usage_{i}.csv')
    df.to_csv(output_file, index=False)
    
print(f"Generated {num_files} CPU usage data files in {output_dir}") 