...
# 创建输出目录
output_dir = "data/latency_benchmark"
os.makedirs(output_dir, exist_ok=True)

def generate_latency_data(num_points, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    # 生成正常范围的请求延迟（100-200ms）
    values = np.random.uniform(100.0, 200.0, num_points)
    
    # 添加较小的随机波动（标准差10ms，因为延迟通常会有一定波动）
    values += np.random.normal(0, 10, num_points)
    ...
    high_anomaly_bases = np.random.uniform(500.0, 800.0, num_anomaly_periods)
    ...
    for i in range(num_anomaly_periods):
        ...
        # 生成高延迟异常 (500-800ms)，在基准值附近波动
        base_value = high_anomaly_bases[i]
        values[anomaly_start:anomaly_start + anomaly_duration] = base_value + np.random.uniform(-30, 30, anomaly_duration)
...
for i in range(1, num_files + 1):
    ...
    df = generate_latency_data(num_points, seed=i)  # 使用文件索引作为随机种子
    
    # 保存为CSV文件
    output_file = os.path.join(output_dir, f'latency_{i}.csv')
    df.to_csv(output_file, index=False)
    
print(f"Generated {num_files} latency data files in {output_dir}") 